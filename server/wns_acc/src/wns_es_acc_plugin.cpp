...
static const char *special_get_response_str = "HTTP/1.1 200 OK\r\n"
...
static const char *http_fake_header = "HTTP/1.1 200 OK\r\n"
		"Server: WNS-ACC/0.1\r\n"
...
static std::map<uint64_t, void *> wns_async_uin_map;

static void wns_acc_free_task_struct(wns_acc_task_struct *tsp);
static void wns_acc_logSession(ENUM_WNS_ERRCODE retcode, eservice_unit_t *eup,
		QMF_PROTOCAL::QmfAccHead *acc_head, QMF_PROTOCAL::QmfHead *qmf_head,
		wns_acc_task_struct *tsp, int force_log = 0);
...
static void tsp_timeout_process(wns_acc_task_struct *tsp, void *arg) {
	int res = -255;
	uint32_t i, j;
...
//    }
	...
	if (tsp->client_id > 0) {
		std::map<uint64_t, void *>::iterator the_iter = wns_async_map.find(
				tsp->client_id);
		if (the_iter != wns_async_map.end() && the_iter->second != NULL) {
			g_dbg("async tsp timeout, %lu -> %p, eup: %p",
					the_iter->first, the_iter->second, (eservice_user_eup_from_index(get_eup(tsp->client_id))));
...
static int tsp_is_timeout(wns_acc_task_struct *tsp, void *arg) {
...
static void __cond_del_waiting_list(ss_list_head_struct *hsp,
		int (*need_del)(wns_acc_task_struct *, void *), void *need_del_arg,
		void (*cleanup)(wns_acc_task_struct *, void *), void *arg) {
...
static void one_sec_timer_cb(int arg1, short arg2, void *arg) {
	struct eservice_manager_t *egr = (struct eservice_manager_t *) arg;
	struct timeval one_sec_timer_to = { 1, 0 };
	int res = -255;

	g_warn("---- task:");
	g_warn("free: %u, waiting: %u evbuff:%d",
			ss_list_info_head_struct(&wns_acc_task_struct_head), ss_list_info_head_struct(&wns_acc_task_struct_inuse_list), evbuff_num);
	g_warn("-----------");

	__cond_del_waiting_list(&wns_acc_task_struct_inuse_list, tsp_is_timeout,
			(void *) WNS_ACC_MAX_TSP_PROCESS_SECONDS, tsp_timeout_process,
			NULL); /* tsp 10 seconds timeout */

	/* reset timer */
	eservice_user_timer(egr, &one_sec_timer, &one_sec_timer_to,
			one_sec_timer_cb, egr);
	return;
}

static void statistic_timer_cb(int arg1, short arg2, void *arg) {
	struct eservice_manager_t *egr = (struct eservice_manager_t *) arg;
	...
	g_warn("---- statistic:");
	g_warn("req: %u, rsp: %u, disp req: %u, disp rsp: %u, push: %u",
			wns_30s.req_num, wns_30s.rsp_num, wns_30s.dispatch_req_num, wns_30s.dispatch_rsp_num, wns_30s.push_num);
	for (i = 0; i < WNS_ACC_MAX_LISTEN_PORT; i++) {
		if (wns_30s.port_statistic_array[i].used == 0)
			break;

		g_warn("port: %u, bin prot num: %u, http prot num: %u",
				wns_30s.port_statistic_array[i].port, wns_30s.port_statistic_array[i].bin_prot_num, wns_30s.port_statistic_array[i].http_prot_num);
	}

	g_warn("---- push statistic:");
	g_warn(
			"total: %u, <30s: %u:%u, <60s: %u:%u, <180s: %u:%u, <300s: %u:%u, <600s: %u:%u, <900: %u:%u, <1200: %u:%u, <1500: %u:%u",
			push_statistic.push_num, push_statistic.less_30, push_statistic.init_less_30, push_statistic.s_30_60, push_statistic.init_s_30_60, push_statistic.s_60_180, push_statistic.init_s_60_180, push_statistic.s_180_300, push_statistic.init_s_180_300, push_statistic.s_300_600, push_statistic.init_s_300_600, push_statistic.s_600_900, push_statistic.init_s_600_900, push_statistic.s_900_1200, push_statistic.init_s_900_1200, push_statistic.s_1200_1500, push_statistic.init_s_1200_1500);
	g_warn("-----------");
	...
	g_warn("user distriute 15~20min:%u, 10~15min:%u, 5~10min:%u, 0~5min:%u",
			wns_user_distribute.min15_20_user_ilde_num, wns_user_distribute.min10_15_user_ilde_num, wns_user_distribute.min5_10_user_ilde_num, wns_user_distribute.min0_5_user_ilde_num);

	g_warn("---- ------");

	g_warn("---- clear first_nodata statistic:");
	g_warn(
			"clear first_nodata:%u, commm_idle num:%u, fast_idle_num:%u, highest_idle_num:%u",
			wns_idle_statisic.first_no_data_num, wns_idle_statisic.comm_idle_time_num, wns_idle_statisic.fast_idle_time_num, wns_idle_statisic.highest_idle_time_num);
	g_warn("---- ------");
	...
	"	wns_thttp_conn:%u [{common_http_conn:%u} {ping_http_conn :%u}] \n",
			wns_connection_statisic.bin_comm_connection_num + wns_connection_statisic.bin_pin_connection_num + wns_connection_statisic.http_comm_connection_num + wns_connection_statisic.http_pin_connection_num, wns_connection_statisic.bin_comm_connection_num + wns_connection_statisic.bin_pin_connection_num, wns_connection_statisic.bin_comm_connection_num, wns_connection_statisic.bin_pin_connection_num, wns_connection_statisic.http_comm_connection_num + wns_connection_statisic.http_pin_connection_num, wns_connection_statisic.http_comm_connection_num, wns_connection_statisic.http_pin_connection_num);
	g_warn("wns_total_recvied data:%u \n"
	"	{wns_tcp_recvied data:%u} {wns_http_recvied data:%u} \n",
			wns_connection_statisic.bin_client_data_recved + wns_connection_statisic.http_client_data_recved, wns_connection_statisic.bin_client_data_recved, wns_connection_statisic.http_client_data_recved);
	g_warn("-----------");
	...
	/* reset timer */
	eservice_user_timer(egr, &statistic_timer, &statistic_timer_to,
			statistic_timer_cb, egr);
...
static int prepend_proxy_header(wns_acc_task_struct *tsp, eservice_buf_t *ebp) {
	// 如果是oc点加速过来的,需要加上相应的头
	if (tsp && tsp->client_port != 0) {
		wns_cdnproxy_psy_header_t cdn_header;
		cdn_header.Version = 0x82;
		cdn_header.TTL = 32;
		cdn_header.Header_length = (sizeof(cdn_header)
...
static int wns_acc_init_task_struct_list(uint32_t number, uint64_t *min_val,
		uint64_t *max_val) {
	uint32_t i;
	wns_acc_task_struct *tsp = NULL;
	...
	dlmalloc_dbg(task_struct_chunk, number * sizeof(wns_acc_task_struct),
			wns_acc_task_struct*);
...
static void wns_acc_add_waiting_task_struct(wns_acc_task_struct *tsp) {
	uint32_t i;
...
static void wns_acc_del_waiting_task_struct(wns_acc_task_struct *tsp) {
	if (!tsp)
		return;
...
static uint32_t wns_acc_info_task_struct(void) {
	return ss_list_info_head_struct(&wns_acc_task_struct_head);
}

static int wns_acc_sacagent_check(eservice_unit_t *eup,
		QMF_PROTOCAL::QmfAccHead *acc_head, QMF_PROTOCAL::QmfHead *qmf_head,
...
static void wns_acc_logSession(ENUM_WNS_ERRCODE retcode, eservice_unit_t *eup,
	...
	if (acc_head) {
		...
		obj.setTimecost(
				((uint64_t) (g_current_tv.tv_sec * 1000)
						+ (uint64_t) (g_current_tv.tv_usec / 1000))
						- acc_head->Ticks);
...
static void wns_acc_logSession_push(ENUM_WNS_ERRCODE retcode,
		eservice_unit_t *eup, QMF_PROTOCAL::QmfAccHead *acc_head,
	...
	user_context =
			(wns_acc_connect_user_context_t *) eservice_user_conn_get_user_data(
					eup);
...
static void wns_acc_LogDownstreamInfo(QMF_PROTOCAL::QmfHead &qmfhead,
		QMF_PROTOCAL::QmfAccHead &acchead) {
	return;
}

static int extra_indenpent(QMF_PROTOCAL::QmfUpstream &up,
		QMF_PROTOCAL::QmfHead &qmfhead, int &need_replace) {
	wup::UniAttribute<> una;
	if (up.Extra.size() > 0) {
		if (up.BusiControl.noexit == false) {
			una.decode(up.Extra);
			if (!una.containsKey(QMF_PROTOCAL::KEY_EXTRA_BUSICOMPCONTROL)) {
				una.put<QMF_PROTOCAL::QmfBusiControl>(
						QMF_PROTOCAL::KEY_EXTRA_BUSICOMPCONTROL,
						up.BusiControl);
				una.encode(up.Extra);
				need_replace = 1;
		...
		if (up.BusiControl.noexit == false) {
			una.put<QMF_PROTOCAL::QmfBusiControl>(
					QMF_PROTOCAL::KEY_EXTRA_BUSICOMPCONTROL, up.BusiControl);
			una.encode(up.Extra);
			need_replace = 1;
...
static int servicemap_process(QMF_PROTOCAL::QmfUpstream &up,
		int &need_replace) {
...
static int session_prcoess(eservice_unit_t *eup, QMF_PROTOCAL::QmfUpstream &up,
		QMF_PROTOCAL::QmfHead &qmfhead, wns_acc_task_struct *tsp,
		int &need_repack) {
	wns_acc_connect_user_context_t *user_context = NULL;
	if (up.sessionID != 0) {
		...
		if (user_context) {
			...
			if (user_context->session_hash != up.sessionID) {
				if (up.Qua.size() >= 2 && up.DeviceInfo.size() >= 2
						&& up.Token.Key.size() >= 2) {
					g_dbg("update session hash, old: %u",
							user_context->session_hash);
					...
					g_warn("session not found, uin=%lu, cmd=%s, qua=%s",
							up.Uin, up.ServiceCmd.c_str(), user_context->last_qua.c_str());
				...
				g_dbg(
						"need replace for hash, qua size: %zu, deviceinfo size: %zu, A2 size: %zu",
...
static int wns_acc_LogUpstreamInfo(eservice_unit_t *eup,
		QMF_PROTOCAL::QmfHead &qmfhead, QMF_PROTOCAL::QmfAccHead &acchead,
		wns_acc_task_struct *tsp, eservice_buf_t **ebpp) {
	return 0;
...
static int wns_acc_response_push_client_invalid(eservice_unit_t *eup,
		wns_acc_task_struct *tsp, QMF_PROTOCAL::QmfAccHead *accheadp,
		int acchead_len) {
	...
	g_dbg("=> wns_acc_response_push_client_invalid(), eup: %p, tsp: %p",
			eup, tsp);
	...
	if (eservice_buf_add(send_ebp, accheadp, acchead_len) < 0
			|| eservice_buf_add(send_ebp, &qmfhead, sizeof(qmfhead)) < 0) {
...
static int wns_acc_response_push_client_ok(eservice_unit_t *eup,
		wns_acc_task_struct *tsp, QMF_PROTOCAL::QmfAccHead *accheadp,
		int acchead_len) {
	struct eservice_buf_t *send_ebp = NULL;
	int res = -255;
...
static int wns_acc_get_proper_async(eservice_unit_t *eup,
		struct eservice_buf_t *pkg_data, ssize_t pkg_len,
		eservice_unit_t **eupp, wns_acc_task_struct **tspp, int &broadcastnum) {
	int res = -255;
	...
	*tspp = NULL;
	...
	/*************Deco AccHead **********/
	res = m_protocal_parser.DecodeQmfAccHead((const char *) data_start, pkg_len,
			_acc_head);
	if (res != 0) {
		wns_acc_logSession(WNS_CODE_ACC_SERVER_ACCUNPACK, eup, NULL, NULL,
				NULL);
	...
	g_msg(
			"Decode QmfAcc[len=%d ver=%d ip=%u port=%d cliendid=%lu seq=%d accflag:%d]",
			_acc_head.Len, _acc_head.Ver, _acc_head.ClientIP, _acc_head.ClientPort, _acc_head.ClientID, _acc_head.Seq, _acc_head.Flag);
	...
	if (the_iter != wns_async_map.end() && the_iter->second != NULL) {
		*tspp = (wns_acc_task_struct *) (the_iter->second);
		...
		//wns_acc_logSession(WNS_CODE_ACC_CLIENT_INVAID, eup, &_acc_head,
		&_qmf_head, NULL
		);
		return -3;
	}
	for (int i = 0; i < _acc_head.ClientNum; i++) {
		std::map<uint64_t, void *>::iterator the_uin_iter =
				wns_async_uin_map.find(_acc_head.ClientUinList[i]);
		if (the_uin_iter
				!= wns_async_uin_map.end() && the_uin_iter->second != NULL) {
			eupp[i] = (eservice_unit_t*) the_uin_iter->second;
			broadcastnum++;
		} else {
			g_warn("invalid async, not found uin: %lu connection",
					_acc_head.ClientUinList[i]);
		}
	}
...
//    }
	...
	eservice_buf_drain(pkg_data, _acc_head_len); //移除acc头，保留qmf头然后完全透传给client
	ss_list_push_head_struct(&((*tspp)->output_ebp_list),
			eservice_buf_buf2list(pkg_data));
...
static int wns_acc_get_push_client(eservice_unit_t *eup,
		struct eservice_unit_t **eupp, wns_acc_task_struct *tsp) {
	int res = -255;
	QMF_PROTOCAL::QmfAccHead _acc_head;
	QMF_PROTOCAL::QmfHead _qmf_head;
	QMF_PROTOCAL::QmfHeadParser m_protocal_parser;
	...
	*eupp = eservice_user_eup_from_index(idx);
	if (eservice_user_eup_is_invalid(*eupp)) {
		...
		wns_acc_response_push_client_invalid(eup, tsp, &_acc_head,
				_acc_head_len);
		return -2;
	}
	...
	res = m_protocal_parser.DecodeQmfHead(
			(const char *) data_start + _acc_head_len, pkg_len - _acc_head_len,
			_qmf_head);
	if (res != 0) {
		g_warn("Decode QmfHead ERR. ret=%d", res);
		return -2;
	...
	g_msg("Ready send 2 client. sendlen=%zd",
...
static int wns_acc_get_L5(int cmd) {
	int res = -255;
	std::string errmsg;
...
static int wns_acc_update_failed_L5(QOSREQUEST &qos, int usetime_usec) {
	std::string errmsg;
	g_warn("update failed L5, mod id: %d, cmd id: %d, ip: %s, port: %u",
...
static int wns_acc_update_success_L5(QOSREQUEST &qos, int usetime_usec) {
	...
	g_dbg("update successful L5, mod id: %d, cmd id: %d, ip: %s, port: %u",
			qos._modid, qos._cmd, qos._host_ip.c_str(), qos._host_port);
	if (usetime_usec > 0)
...
static int wns_make_empty_response(wns_acc_task_struct *tsp, int wns_code) {
	...
	res = QMF_PROTOCAL::struct_QmfDownstream_pack(&qmfrsp, busi_buf,
			&qmfbodylen, NULL);
	if (res) {
	...
	m_protocal_parser.EncodeQmfHead((char *) pkg_buf, &qmfbufflen,
			_rep_qmf_head, (char *) busi_buf, qmfbodylen);
...
static eservice_callback_return_val dispatch_long_conn_data_arrive_cb(
	...
	while (1) {
		...
		if (buf_data_len
				< (ssize_t) sizeof(wns_inner_psy_header_t)|| buf_data_len > WNS_ACC_MAX_VALID_PKG_LEN) {
		...
		if (pkg_len
				<= (ssize_t) sizeof(wns_inner_psy_header_t)|| pkg_len > WNS_ACC_MAX_VALID_PKG_LEN) {
		...
		if (the_tsp && the_tsp->output_ebp_list.number > 0) {
			...
			if (dptr) {
				...
				for (int i = 0; i < broadcast_num; i++) {
					send_ebp=eservice_buf_new(eservice_buf_datalen(the_ebp));
					if(eservice_buf_copy_data(send_ebp,the_ebp)){
...
//    }
	...
	if (tsp && tsp->client_info_p) {
		tsp->client_info_p->server_ip =
				ntohl((uint32_t)(inet_addr(qos_dispatch._host_ip.c_str())));
...
//    }
	...
	if (!dispatch_long_eup) {
		dispatch_long_eup = eservice_user_register_tcp_conn(NULL,
				qos_dispatch._host_ip.c_str(), qos_dispatch_port_str, 1000,
				&to);
		if (!dispatch_long_eup) {
		...
		g_dbg("new dispatch connection: %p app type:%d",
				dispatch_long_eup, app_type);

		eservice_user_set_tcp_rw_buffer_size(dispatch_long_eup, 2 * 1024 * 1024,
				2 * 1024 * 1024); /* 2M */
		eservice_user_set_tcp_conn_data_arrive_cb(dispatch_long_eup,
				dispatch_long_conn_data_arrive_cb, (void *) dispatch_long_eup);
		eservice_user_set_tcp_conn_terminate_cb(dispatch_long_eup,
				dispatch_long_conn_terminate_cb, (void *) dispatch_long_eup);
		...
		g_dbg("reuse dispatch connection: %p app type:%d",
				dispatch_long_eup, app_type);
	}
...
static int wns_connect_to_dispatch_and_send_data(eservice_unit_t *eup,
	...
	if (send_ebp) {
		...
		if (res < 0) {
			// only write failed
			if (res == eservice_user_errcode_write)
...
static int wns_pkg_error_data(eservice_unit_t *eup, wns_acc_task_struct *tsp,
		QMF_PROTOCAL::QmfHeadParser &m_protocal_parser,
		QMF_PROTOCAL::QmfHead &_qmf_head, const int error_code) {
	struct dbl_list_head *ptr = NULL, *n = NULL;
	struct eservice_buf_t *ebp = NULL;
	QMF_PROTOCAL::QmfHead _rep_qmf_head;
	QMF_PROTOCAL::QmfDownstream qmfrsp; /* dummy */
	...
	ss_list_push_head_struct(&(tsp->output_ebp_list),
			eservice_buf_buf2list(ebp));
	return 0;
}

static int wns_staticis_connection_and_recived_num(eservice_unit_t *eup,
		wns_acc_task_struct *tsp, const QMF_PROTOCAL::QmfHead _qmf_head) {
	wns_acc_connect_user_context_t *user_context =
			(wns_acc_connect_user_context_t *) eservice_user_conn_get_user_data(
					eup);
...
static int wns_acc_process_pkg(eservice_unit_t *eup, wns_acc_task_struct *tsp) {
	int res = -255;
	ssize_t input_len = 0;
	QMF_PROTOCAL::QmfHead _qmf_head;
	QMF_PROTOCAL::QmfHeadParser m_protocal_parser;
	...
	g_dbg("=> wns_acc_process_pkg(), tsp: %p, input ebp: %p",
			tsp, tsp->input_ebp);
	...
	_qmf_head.init();
	data_start = eservice_buf_start(tsp->input_ebp);
	res = m_protocal_parser.DecodeQmfHead((const char*) data_start,
			(unsigned int) input_len, _qmf_head);
	if (res != 0) {
	...
	g_msg(
			"decode QmfHead[len=%d ver=%d cmd=%d  Key=%lu uin=%lu ,reserveflag=%u, bodylen=%d  seq=%d]",
	...
	if (user_context) {
		user_context->datacomed = 1;
		if(user_context->uin!=_qmf_head.Uin && user_context->uin>1000){
			std::map<uint64_t, void *>::iterator the_uin_iter =
							wns_async_uin_map.find(user_context->uin);
			wns_async_uin_map.erase(the_uin_iter);
			//如果用户切换账号过来了，并且老的对应关系还在，就移除
		}
		user_context->uin=_qmf_head.Uin;
		//连接和uin对应
		eservice_user_conn_set_user_data(eup, user_context);
	}
	...
	if (_qmf_head.BodyLen == 0) {
		g_warn(
				"receive a NOT-PING package with qmfhead.bodylen==0 uin=%lu qua=%s",
				_qmf_head.Uin, (user_context ? user_context->last_qua.c_str() : ""));

		return wns_pkg_error_data(eup, tsp, m_protocal_parser, _qmf_head,
				WNS_CODE_ACC_SERVER_ERROR_DATA);
	}
...
//#endif
	...
	_acc_head.ClientIP =
			tsp->client_ip > 0 ?
					htonl(tsp->client_ip) :
					eservice_user_get_peer_addr_network(eup);
...
//    }
	...
	if (tsp->need_not_rsp == 0)
		wns_30s.req_num++;
	eservice_buf_move_data(to_dispatch_ebp, tsp->input_ebp, -1); //(to_dispatch_ebp,tsp->input_ebp)
//#if 0
...
//    }
	...
	_acc_head.Len =
			htonl(eservice_buf_datalen(to_dispatch_ebp) + _acc_head_len);
	if (eservice_buf_prepend(to_dispatch_ebp, &_acc_head, _acc_head_len) < 0) {
		g_warn("eservice_buf_add() failed: %s", strerror(errno));
		ESERVICE_DESTROY_EBP(to_dispatch_ebp);
		wns_acc_logSession(WNS_CODE_ACC_SEND2_DISP_FAILED, eup, NULL, NULL,
				NULL, 1);
		return -1;
	}
	...
	if (res < 0) {
		g_warn("wns_connect_to_dispatch_and_send_data() failed: %d", res);
		ESERVICE_DESTROY_EBP(to_dispatch_ebp);
		wns_acc_logSession(WNS_CODE_ACC_DISPACH_SND_ERROR, eup, &_acc_head,
				&_qmf_head, tsp, 1);

		return wns_pkg_error_data(eup, tsp, m_protocal_parser, _qmf_head,
				WNS_CODE_ACC_DISPACH_SND_ERROR);
	}
	...
	if (tsp->need_not_rsp == 0) {
		...
		wns_async_uin_map[_qmf_head.Uin] = (void *) tsp->eup;
	}
...
extern "C" eservice_callback_return_val eservice_cb_init(
	...
	int res = -255;
	g_dbg("=> eservice_cb_init(), eup: %p, earg: %p, garg: %p",
			eup, earg, garg);
	...
	g_wns_acc_main_config.task_num = (uint32_t) atoi(
			config["server.task_num"].c_str());
	g_wns_acc_main_config.dest_modid = atoi(
			config["server.dest_modid"].c_str());
	g_wns_acc_main_config.dest_cmd = atoi(config["server.dest_cmd"].c_str());

	g_wns_acc_main_config.other_dest_modid = atoi(
			config["server.other_dest_modid"].c_str());
	g_wns_acc_main_config.other_dest_cmd = atoi(
			config["server.other_dest_cmd"].c_str());

	g_wns_acc_main_config.client_retry = atoi(
			config["server.client_retry"].c_str());
	g_wns_acc_main_config.idle_clean_seconds = atoi(
			config["server.idle_clean_seconds"].c_str());
	g_wns_acc_main_config.fast_idle_clean_seconds = atoi(
			config["server.fast_idle_clean_seconds"].c_str());
	g_wns_acc_main_config.highest_idle_clean_seconds = atoi(
			config["server.highest_idle_clean_seconds"].c_str());
	g_wns_acc_main_config.firstnodata_idle_clean_seconds = atoi(
			config["server.firstnodata_idle_clean_seconds"].c_str());
	g_wns_acc_main_config.sac_agent_flag = atoi(
			config["server.sac_agent_flag"].c_str());
	g_sac_dcagent_flag = atoi(config["server.sac_dcagent_flag"].c_str());
	g_wns_acc_main_config.clean_clog = atoi(
			config["server.clean_clog"].c_str());
	g_wns_acc_main_config.max_pkg_size = atoi(
			config["server.max_pkg_size"].c_str());
	g_wns_acc_main_config.max_group_user_num = atoi(
			config["server.max_group_user_num"].c_str());
	if (g_wns_acc_main_config.max_group_user_num
			<= 0|| g_wns_acc_main_config.max_group_user_num >DEFAULT_MAX_GROUP_USER_NUM)
		g_wns_acc_main_config.max_group_user_num = 8;

	if (g_wns_acc_main_config.idle_clean_seconds <= 0)
		g_wns_acc_main_config.idle_clean_seconds =
				WNS_ACC_DEFAULT_CONN_IDLE_SECONDS;
	if (g_wns_acc_main_config.fast_idle_clean_seconds <= 0)
		g_wns_acc_main_config.fast_idle_clean_seconds =
				WNS_ACC_DEFAULT_CONN_IDLE_SECONDS;
	if (g_wns_acc_main_config.highest_idle_clean_seconds <= 0)
		g_wns_acc_main_config.highest_idle_clean_seconds =
				WNS_ACC_DEFAULT_CONN_IDLE_SECONDS;
	if (g_wns_acc_main_config.firstnodata_idle_clean_seconds <= 0)
		g_wns_acc_main_config.firstnodata_idle_clean_seconds =
				WNS_ACC_DEFAULT_CONN_IDLE_SECONDS;
	...
	g_dbg( "task num: %u, dest modid: %d, dest cmd: %d, idle clean seconds: %d",
			g_wns_acc_main_config.task_num, g_wns_acc_main_config.dest_modid, g_wns_acc_main_config.dest_cmd, g_wns_acc_main_config.idle_clean_seconds);
	g_dbg("clean_clog flag:%d", g_wns_acc_main_config.clean_clog);
	g_dbg("other_dest modid: %d, other_dest cmd: %d max_pkg:%d",
			g_wns_acc_main_config.other_dest_modid, g_wns_acc_main_config.other_dest_cmd, g_wns_acc_main_config.max_pkg_size);
	g_dbg("init ok");
...
extern "C" eservice_callback_return_val eservice_cb_pre_run(
	...
	g_dbg("=> eservice_cb_pre_run(), eup: %p, earg: %p, garg: %p",
			eup, earg, garg);
...
extern "C" eservice_callback_return_val eservice_cb_after_accept(
	...
	g_dbg("=> eservice_cb_after_accept(), eup: %p, earg: %p, garg: %p",
			eup, earg, garg);
...
static int eservice_user_distribute_staticis(eservice_unit_t *eup) {
	if (eservice_user_conn_idle_seconds(eup) >= 900
			&& eservice_user_conn_idle_seconds(eup) < 1200) {
		wns_user_distribute.min15_20_user_ilde_num++;
	} else if (eservice_user_conn_idle_seconds(eup) >= 600
			&& eservice_user_conn_idle_seconds(eup) < 900) {
		wns_user_distribute.min10_15_user_ilde_num++;
	} else if (eservice_user_conn_idle_seconds(eup) >= 300
			&& eservice_user_conn_idle_seconds(eup) < 600) {
		wns_user_distribute.min5_10_user_ilde_num++;
	} else if (eservice_user_conn_idle_seconds(eup) >= 0
			&& eservice_user_conn_idle_seconds(eup) < 300) {
...
extern "C" eservice_callback_return_val eservice_cb_tcp_conn_timeout(
	...
	if (user_context->datacomed == 0) {
		if (eservice_user_conn_idle_seconds(eup)
				>= g_wns_acc_main_config.firstnodata_idle_clean_seconds) {
			string client_ip = TOOL::Int2Str(
					(eservice_user_get_peer_addr_network(eup)));
			g_warn(
					"client connection first not datacome cut, eup: %p user_context ip:%s",
					eup, client_ip.c_str());
	...
	if (g_total_connection_num > (g_max_connection_num * 95 / 100)) {
		if (eservice_user_conn_idle_seconds(eup)
			...
			wns_acc_logSession(WNS_CODE_OVERLOAD_ACTIVE_CLOSE, eup, NULL, NULL,
					NULL);
			goto cleaneup;
		}
	}
...
extern "C" eservice_callback_return_val eservice_cb_tcp_terminate(
	...
	g_dbg("=> eservice_cb_tcp_terminate(), eup: %p, earg: %p, garg: %p",
			eup, earg, garg);
...
extern "C" eservice_callback_return_val eservice_cb_data_arrive(
	...
	g_dbg("=> eservice_cb_data_arrive(), eup: %p, earg: %p, garg: %p",
			eup, earg, garg);
	...
	while (1) {
		...
		buf_data_len = eservice_user_datalen(eup);
		if (buf_data_len < 0) {
			wns_acc_logSession(WNS_CODE_ACC_SERVER_ERROR_DATA, eup, NULL, NULL,
					NULL, 1);
			return eservice_cb_data_arrive_fatal;
		}

		if (buf_data_len
				< (ssize_t) sizeof(wns_psy_header_t)|| buf_data_len > WNS_ACC_MAX_VALID_PKG_LEN) {
			return eservice_cb_data_arrive_not_enough;
		}
...
//        }
		psy_headp = (wns_psy_header_t *) (eservice_user_pos_ex(eup, 0,
				(ssize_t) (sizeof(wns_psy_header_t))));
		if (psy_headp == NULL) {
			g_warn("user_pos_ex return NULL");
			return eservice_cb_data_arrive_fatal;
		}
		...
		if (psy_headp->wns_protocol.Wns_sign == WNS_SIGN_INT) {
			...
			if (pkg_len
					<= (ssize_t) (sizeof(wns_psy_header_t))|| pkg_len > WNS_ACC_MAX_VALID_PKG_LEN) {
				g_warn("invalid wns pkg len: %zd", pkg_len);
				wns_acc_logSession(WNS_CODE_ACC_SERVER_ERROR_DATA, eup, NULL,
						NULL, NULL, 1);
				return eservice_cb_data_arrive_fatal;
			}
		...
		//广播逻辑处理
		else if (psy_headp->wns_push_protocol.SOH == WNS_INNER_SIGN) { /* push server data */
			client_eup = NULL;
			...
			g_warn("unknown protocol, [%x %x %x %x]",
					psy_headp->wns_post_protocol.post[0], psy_headp->wns_post_protocol.post[1], psy_headp->wns_post_protocol.post[2], psy_headp->wns_post_protocol.post[3]);
			return eservice_cb_data_arrive_fatal;
		}
