...
int
test_tcp_listen_socket_reuseport()
{
    int fd, res;
...
int
set_dontfragment(int sockfd, int flag)
{
	int val;
	...
	return setsockopt(sockfd, IPPROTO_IP, IP_MTU_DISCOVER, &val, sizeof(val));
}

int
set_maxseg_size(int sockfd, uint32_t max_seg_size)
{
	int val;
	
	val = (int)max_seg_size;
	
	return setsockopt(sockfd, IPPROTO_TCP, TCP_MAXSEG, &val, sizeof(val));
}

int
set_tcp_nodelay(int sockfd, int is_nodelay)
{
	return setsockopt(sockfd, IPPROTO_TCP, TCP_NODELAY,(char *)&is_nodelay,sizeof(is_nodelay));
}
...
static int
_create_tcp_listen(const struct sockaddr_in *srvp, int nonblock, int reuseport)
{
	int fd, res;
    ...
    if (reuseport)
    {
        if (reuse_port(fd) != 0)
        {
            g_warn("setsocketopt SO_REUSEPORT failed: %s", strerror(errno));
            return -2;
        }
    }
    else
	...
	//set_maxseg_size(fd, 1200);
	set_recvbuf_size(fd, 4096);
	set_sendbuf_size(fd, 4096);
	...
	if (res < 0) {
		g_warn("bind() failed: %s, ip: %s, port: %u", 
               strerror(errno), inet_ntoa(*((struct in_addr *)(&srvp->sin_addr.s_addr))), ntohs(srvp->sin_port));
		close(fd);
		return -2;
	}
...
int
create_tcp_listen_socket(const char * ip, const char * port, int nonblock, int reuseport)
{
	...
	return _create_tcp_listen(&srvaddr, nonblock, reuseport);
}

int
create_tcp_listen_socket_struct(const struct sockaddr_in *srvp, int nonblock, int reuseport)
{
	struct sockaddr_in srvaddr;
...
static int
_create_udp_listen(const struct sockaddr_in * srvp, int nonblock, int reuseport)
{
	int fd, res;
...
int
create_udp_listen_socket(const char *ip, const char *port, int nonblock, int reuseport)
{
	struct sockaddr_in srv;	
	...
	return _create_udp_listen(&srv, nonblock, reuseport);
}

int
create_udp_listen_socket_struct(const struct sockaddr_in * srvp, int nonblock, int reuseport)
{
	struct sockaddr_in srvaddr;
	...
	return _create_udp_listen(&srvaddr, nonblock, reuseport);
}
