...
#define	close_socket(f)	do {if ((f) >= 0) {close((f)); (f) = -1;}} while (0)
...
extern int set_dontfragment(int sockfd, int flag);
extern int set_maxseg_size(int sockfd, uint32_t max_seg_size);
extern int set_tcp_nodelay(int sockfd, int is_nodelay);
...
extern int create_tcp_listen_socket_struct(const struct sockaddr_in *srv, int nonblock, int reuseport);
...
extern int create_udp_listen_socket(const char *ip, const char *port, int nonblock, int reuseport);
extern int create_udp_listen_socket_struct(const struct sockaddr_in *srv, int nonblock, int reuseport);
extern void print_mem(unsigned char* buf, int len, int show);
extern int test_tcp_listen_socket_reuseport();
